...
export default function AddEdit({ product, categories, onSave, onClose }) {
	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-slate-900/50 p-2 sm:p-4 md:p-6 backdrop-blur-xs">
			<form onSubmit={handleSubmit} className="w-full max-w-full rounded-2xl border border-slate-200 bg-white shadow-2xl">
				...
				<div className="grid gap-5 p-6 sm:grid-cols-2 lg:grid-cols-4">
					...
					<div className="sm:col-span-2 lg:col-span-2"><label htmlFor="product-image" className="mb-2 block text-sm font-medium text-slate-700">Product image</label><div className="flex items-center gap-4"><label htmlFor="product-image" className="flex h-24 w-24 shrink-0 cursor-pointer items-center justify-center overflow-hidden rounded-xl border border-dashed border-slate-300 bg-slate-50 text-slate-400 transition hover:border-emerald-500 hover:text-emerald-600">{imagePreview ? <img src={imagePreview} alt="Product preview" className="h-full w-full object-cover" /> : <ImagePlus size={24} />}</label><div className="flex-1"><input id="product-image" name="image" type="file" accept="image/jpeg,image/png,image/webp" onChange={handleChange} className="block w-full text-sm text-slate-500 file:mr-3 file:rounded-md file:border-0 file:bg-emerald-50 file:px-3 file:py-2 file:font-medium file:text-emerald-700" /><p className="mt-2 text-xs text-slate-400">JPG, PNG or WEBP. Choose an image to show it in the product table.</p></div></div></div>
				</div>
